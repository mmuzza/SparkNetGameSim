...
// Task #3 --> Calling SimRank on the nodes from cartesian product of perturbed (Random Walk) and Original
// This class runs Spark on the file of nodes generated by Random Walk Through on Perturbed
object SparkSimRank {
  ...
  def executeSimRankOnRandomWalk(): Unit = {

    // Create a SparkConf and SparkContext
    val conf = new SparkConf()
      .setAppName("SimRankApp")
      .setMaster("local") // Using "local" for local mode. Can specify Spark cluster here
    ...
    // Categorizing data based on score
    // If score is below 0.9 it will save those nodes in a txt
    // If score is above 0.9 it will save those nodes in a txt
    // If score is equal to 0.9 it will save those nodes in a txt
